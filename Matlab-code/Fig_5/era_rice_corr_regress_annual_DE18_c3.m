%%%%%%%%%%%%%%%%%%%%%%%%%
% era_rice_corr_regress_annual_DE18_c3.m
% Daniel Emanuelsson
% Matlab 2017a
% Github version 1
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% * fig.m           fileexchange
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  original file era_wais_regression_test.m B Markle UoW
    ...
    for i=1:c1 %360 lon
        for  k=134:241% c2; % lat
         ...
         ind_cp=isnan(ind_psa_sum_nonactive_c(i,k));
            ...
            s1=plotm(double(era_lat(k)), double(era_long(i)),...
                'k','Marker','.','MarkerSize',4);  % marker size change comes through clearer in saved fig
